...
def loadBackground():
    bg_image = Image.open("assets/bank_background.png")
    bg_photo = ImageTk.PhotoImage(bg_image)

    bg_label = tk.Label(app, image=bg_photo)
    bg_label.place(x=-2, y=-2, relwidth=1, relheight=1)
...
def mainPage():
    for widget in app.winfo_children():
        widget.destroy()
    loadBackground()
    tk.Label(app, text="Welcome to the TKinter Banking App", font=("Arial", 24), bg="#ffffff").place(x=700, y=100)
    tk.Button(app, text="Login", font=("Arial", 14), command=login, bg="#0042A5", fg="#FFFFFF").place(x=1800, y=100)
...
def createAccount():
    ...
    tk.Label(app, text="Username:", font=("Arial", 14), bg="#ffffff").place(x=680, y=467)
    tk.Label(app, text="Password:", font=("Arial", 14), bg="#ffffff").place(x=680, y=512)
    tk.Label(app, text="Confirm Password:", font=("Arial", 14), bg="#ffffff").place(x=608, y=544)

    usernameregister = tk.Entry(app, font=("Arial", 14))
    usernameregister.place(x=911, y=480, anchor=tk.CENTER)

    passwordregister = tk.Entry(app, font=("Arial", 14), show="*")
    passwordregister.place(x=800, y=512)

    confpasswordregister = tk.Entry(app, font=("Arial", 14), show="*")
    ...
    def requirements():
        ...
        if text != "✓ Account Created Successfully! Redirecting to Main Page...":
            message_label = tk.Label(app, text=text, font=("Arial", 14), bg="#ff0000", fg="#FFFFFF")
            message_label.place(x=608, y=800)
        else:
            message_label = tk.Label(app, text=text, font=("Arial", 14), bg="#00ff00", fg="#000000")
            message_label.place(x=608, y=800)
            app.after(2000, mainPage)

    register_button = tk.Button(app, text="Register", font=("Arial", 14), command=requirements)
...
def login():
    ...
    register_button = tk.Button(app, text="Login", font=("Arial", 14), command=logineErrors)
    register_button.place(x=800, y=580)
